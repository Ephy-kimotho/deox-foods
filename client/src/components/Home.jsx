  ...
  return (
    <div className="bg-gray-100 dark:bg-night-100 text-gray-900 dark:text-gray-200">
      ...
      <div className="container mx-auto p-4 mb-4">
        <h2 className="text-3xl font-bold text-center mb-4">Nearby Restaurants</h2>
        <div className="p-4 bg-gray-200 rounded-lg">
          <Slider {...settings} className="space-x-0.5 md:space-x-1">
            {dummyRestaurants.map((restaurant) => (
              <div key={restaurant.id} className="flex justify-center">
                <img src={restaurant.image} alt={restaurant.name} className="rounded-full w-16 h-16 md:w-24 md:h-24" />
              </div>
            ))}
          </Slider>
        </div>
